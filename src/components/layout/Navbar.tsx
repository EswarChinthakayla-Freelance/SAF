...
export const Navbar: React.FC = () => {
  ...
  return (
    ...
    >
          ...
          {PUBLIC_NAV_ITEMS.map((link) => {
            ...
            return (
              <Link
                key={link.path}
                to={link.path}
                className={`relative py-1.5 text-[11px] uppercase tracking-[0.18em] font-medium transition-all duration-200 outline-none focus-visible:text-[#E8B84B] ${isActive
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[70px] bg-[#0A0A0A]/98 backdrop-blur-2xl z-50 p-6 flex flex-col justify-between overflow-y-auto animate-fade-in border-t border-[#2A2A2A]">
            ...
            {PUBLIC_NAV_ITEMS.map((link, idx) => {
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-2xl font-serif tracking-wide transition-colors flex items-center justify-between ${isActive ? 'text-[#E8B84B] font-semibold' : 'text-[#D1CCC2] hover:text-[#F5F0E8]'
                    }`}
                >
                  <span>{link.name}</span>
