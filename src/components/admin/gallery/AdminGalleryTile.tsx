...
export const AdminGalleryTile: React.FC<AdminGalleryTileProps> = ({
  ...
  return (
    <article className="bg-[#141414] border border-[#242424] hover:border-[#383838] rounded-none overflow-hidden flex flex-col justify-between transition-all duration-200 group shadow-md hover:shadow-xl">
      {/* Visual Image Stage */}
      <div className="relative aspect-[4/3] bg-[#0A0A0A] overflow-hidden">
        ...
        <div className="absolute top-2.5 right-2.5 flex items-center gap-1">
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono font-medium backdrop-blur-md shadow-sm border ${image.is_active
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              className="w-7 h-7 p-0 inline-flex items-center justify-center bg-[#0A0A0A]/80 hover:bg-[#1A1A1A] text-[#D1CCC2] hover:text-[#F5F0E8] rounded-none backdrop-blur-md border border-[#2E2E2E] cursor-pointer transition-colors"
            >
      ...
      <div className="p-3.5 sm:p-4 space-y-2.5 flex-1 flex flex-col justify-between">
        ...
        <div className="pt-2.5 border-t border-[#222222] flex items-center justify-between gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => navigate(`/admin/gallery/${image.id}/preview`)}
            className="h-7 px-2.5 text-xs font-sans font-medium text-[#C9A84C] hover:text-[#E8B84B] hover:bg-[#1A1A1A] rounded-none transition-colors"
          >
